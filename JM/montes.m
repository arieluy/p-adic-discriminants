...
function pfactordegrees(p, polynomial)
  _,_,_,EF := localmontesmatrix(polynomial,p:generadors:=false,diag:="None");
...
vprint GaloisTower,2: "  ",p,"- adic factorization of polynomial of degree",Degree(polynomial);
  fact := Factorization(polynomial);
vprint GaloisTower,2: "   -- over Z:",[Degree(g[1]):g in fact];
  ret := [];
  for f in fact do
    pfd := pfactordegrees(p,f[1]);
    for i in [1..f[2]] do
       ret cat:= pfd;
    end for;
  end for;
vprint GaloisTower,2: "   -- done:",ret;
  return ret;
end intrinsic;
    ...
    for i:=1 to l do
        if (fa[i,2] eq 1 or Fpy!G mod fa[i,1] ne Fpy!0)
            then
                ...
                primeid`generator:=K!(Coefficients(lifa[i] mod Pol) cat [0: j in [Degree(lifa[i])+2..Degree(K)]]);
                Append(~idealfactors,primeid);
