// from Sebastian Pauli and others

"attaching class_field";
//////////////////////////////
// Automorphisms
//////////////////////////////
// All Extensions
...
//  end if;
  ...
  ret :=  HasRoot(f);
  if ret then 
    "v(disc) =",Valuation(Discriminant(R)),"v diff",Valuation(Discriminant(R)-Discriminant(S));
  end if;
  return ret;
end function;
...
are isomrphic}
  ...
  ret :=  HasRoot(gg);
  if ret then 
    "v(disc) =",Valuation(Discriminant(f)),"v diff",Valuation(Discriminant(f)-Discriminant(g));
  end if;
...
function is_isomorphic_in_list(f,L) // k extension // L list of extensions
  for l in L do lx := PolynomialRing(l);
    if HasRoot(lx!f) then 
    "v(disc) =",Valuation(Discriminant(l)),"v diff",Valuation(Discriminant(l)-Discriminant(f));
    return true; end if; end for;
  return false;
end function;
